...
export default function JobSourcesClient() {
  ...
  // QA-audit fix: auto-detection is correct as far as it goes (URL-pattern
  // detection is reliable; HTML-based detection only fires when a URL
  // doesn't match a known ATS host) - but for ~90% of real-world sources the
  // career page lives on the company's own domain, so the slug ends up
  // either undetectable or a best-effort guess that turns out wrong once the
  // scraper actually hits the ATS API (see the many "slug not found" /
  // "API error 404" entries below). There was previously no way to correct a
  // wrong guess without a code change/direct DB edit. This adds an inline
  // "Edit" action so ATS type + slug can be fixed by hand from the UI.
  const [editingId, setEditingId] = useState<string | null>(null)
  const [editForm, setEditForm] = useState<{ ats_type: string; ats_slug: string }>({ ats_type: '', ats_slug: '' })
  ...
  function startEdit(source: CareerSource) {
    setEditingId(source.id)
    setEditForm({ ats_type: source.ats_type ?? 'html', ats_slug: source.ats_slug ?? '' })
  }
  ...
  async function saveEdit(id: string) {
    ...
    try {
      const res = await fetch(`/api/job-sources/${id}`, {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          ats_type: editForm.ats_type,
          ats_slug: editForm.ats_slug.trim() || null,
          // Clear the stale error so the row doesn't keep showing the old
          // failure until the next scrape confirms the corrected slug.
          last_error: null,
        }),
      })
      if (res.ok) {
        const data = await res.json()
        setSources(s => s.map(src => src.id === id ? { ...src, ...data.source } : src))
        setEditingId(null)
      }
    } catch {
      // leave editor open on network error
    }
    setSavingEdit(false)
  }
...
function SourceTable({
  ...
  editingId, editForm, setEditForm, savingEdit, onStartEdit, onSaveEdit, onCancelEdit,
}: {
  ...
  editForm: { ats_type: string; ats_slug: string }
  setEditForm: React.Dispatch<React.SetStateAction<{ ats_type: string; ats_slug: string }>>
  savingEdit: boolean
  onStartEdit: (source: CareerSource) => void
                  ...
                  <td className="px-4 py-3">
                    {editingId === source.id ? (
                      <div className="flex items-center gap-1.5">
                        <select
                          value={editForm.ats_type}
                          onChange={e => setEditForm(f => ({ ...f, ats_type: e.target.value }))}
                          className="text-xs border rounded-md px-1.5 py-1"
                        >
                          {Object.keys(ATS_BADGE).map(t => (
                            <option key={t} value={t}>{ATS_BADGE[t].label}</option>
                          ))}
                        </select>
                        <input
                          value={editForm.ats_slug}
                          onChange={e => setEditForm(f => ({ ...f, ats_slug: e.target.value }))}
                          placeholder="slug"
                          className="text-xs border rounded-md px-1.5 py-1 w-20"
                        />
                        <button
                          onClick={() => onSaveEdit(source.id)}
                          disabled={savingEdit}
                          title="Save"
                          className="text-xs px-1.5 py-1 rounded-md font-medium"
                          style={{ color: '#059669' }}
                        >
                          ✓
                        </button>
