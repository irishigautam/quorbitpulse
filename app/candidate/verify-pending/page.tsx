...
/**
 * Defensive fallback — requireCandidate() redirects here if a session exists
...
export default function CandidateVerifyPendingPage() {
  ...
  async function handleResend() {
    ...
    if (user?.email) {
      await supabase.auth.resend({
        type: 'signup',
        email: user.email,
        options: { emailRedirectTo: `${window.location.origin}/auth/confirm?next=/candidate/dashboard` },
      })
      setResent(true)
    }
    setLoading(false)
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center px-4">
      <div className="bg-white rounded-2xl shadow-sm border p-8 w-full max-w-md text-center">
        ...
        <p className="text-sm mb-4" style={{ color: 'var(--muted)' }}>
          Your profile needs a confirmed email address before you can access your dashboard.
        </p>
        {resent && (
